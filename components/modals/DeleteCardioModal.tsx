...
import { deleteCardio } from "@/lib/actions/cardio.actions";

export function DeleteCardioModal({
	open,
	cardioId,
	userId,
	closeModal,
	...
	const handleSubmit = async () => {
		try {
			...
			const res: any = await deleteCardio({ userId, cardioId });
			if (res?.status == 400)
				return toast({
					title: "Error!",
			...
			<DrawerContent>
				<div className="mx-auto w-full sm:max-w-sm lg:max-w-lg py-10 container">
					...
					<p className="text-xs leading-loose mt-2 mb-4">
						...
						This action cannot be undone. Once deleted, all
